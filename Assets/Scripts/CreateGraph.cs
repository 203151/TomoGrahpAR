...
public class CreateGraph : MonoBehaviour
{
    ...
    private int maxHeightOfTomograph;
    ...
    public void CreateGraphInPoint()
    {
       ...
       if(CheckPoints(ref point))
        ...
        int numberOfFrame = 0;
        maxHeightOfTomograph = (int)((Mathf.Abs(minGraphValue[(point.x * 32) + point.y]) + Mathf.Abs(maxGraphValue[(point.x * 32) + point.y])) * 1000);
        Texture2D graphImage = new Texture2D(scriptableObject.numberOfFrames, maxHeightOfTomograph);
        ...
        //Color[] color = Enumerable.Repeat(Color.white, scriptableObject.numberOfFrames * maxHeightOfTomograph).ToArray();
        
        //graphImage.tex
        foreach (var value in values)
        {
            
            graphImage.SetPixel(numberOfFrame, (int)((value[(point.x * 32) + point.y] + ( Mathf.Abs(minGraphValue[(point.x * 32) + point.y])) ) * 1000), Color.red);
            numberOfFrame++;
        }
        ...
        graphGameObject.gameObject.GetComponent<Image>().sprite = Sprite.Create(graphImage, new Rect(0, 0, numberOfFrame, maxHeightOfTomograph), new Vector2(0, 0));
        graphGameObject.transform.SetParent(gameObject.transform.parent, false);
        graphGameObject.transform.localScale = new Vector3(0.4f * ((numberOfFrame / 32) + ((float)(numberOfFrame % 32) / 32)), 0.4f * (maxHeightOfTomograph / 320) , graphGameObject.transform.localScale.z);
